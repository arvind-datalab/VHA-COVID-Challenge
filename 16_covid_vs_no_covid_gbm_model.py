...
features_df= pd.read_csv(output_path+'/variable_imp_covid.csv')
...
top_feature_df=topFeatures(features_df,0.97)
top_feature_df1=top_feature_df
top_feature_df1 = top_feature_df1.append({'feature_name' : 'Id'} , ignore_index=True)
top_feature_df2 = top_feature_df1.append({'feature_name' : 'covid_flag'} , ignore_index=True)
...
feat_imp.to_csv(output_path+'/variable_imp_final_covid.csv',index=False)

# generate feature importance count
car_top_df_melt=car_top_df.melt(id_vars =['Id','covid_flag']) 
car_top_df_count=pd.DataFrame(car_top_df_melt.groupby(['variable','covid_flag'])['value'].agg(np.sum)).reset_index()
car_top_df_pivot=car_top_df_count.pivot_table(index=('variable'),columns='covid_flag',aggfunc='sum',fill_value=0).reset_index().rename(columns={"Index": "Index4", 0: "deceased_id"})
car_top_df_pivot.columns = ['variable', 'no_covid', 'covid']
car_top_df_pivot['total_patient']=car_top_df_pivot['no_covid']+car_top_df_pivot['covid']
...
car_idcnt_ft2.to_csv(output_path+'/variable_summary_final.csv')
...
result.to_csv(output_path+'/covid_no_covid_output_input.csv', index = False)
result.to_csv(output_path+'/covid_no_covid_output.csv', index = False,header=False)
