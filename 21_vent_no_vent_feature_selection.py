...
df2= car_df.drop(['LOS_ICU','icu_date','inpatient_date','LOS_HOS','vent_date','Cause of Death [US Standard Certificate of Death]'
                  ,'covid_flag','inpatient_flag','icu_flag','deceased_flag','COVERAGE_PCT'],axis=1)
...
# Feature importance
model = GradientBoostingClassifier(n_estimators=200,learning_rate=0.1,max_depth=7)
...
feat_imp.to_csv(output_path+'/variable_imp_vent.csv')

# generate feature importance count
df2_melt=df2.melt(id_vars =['Id','vent_flag']) 
df2_count=pd.DataFrame(df2_melt.groupby(['variable','vent_flag'])['value'].agg(np.sum)).reset_index()
df2_pivot=df2_count.pivot_table(index=('variable'),columns='vent_flag',aggfunc='sum',fill_value=0).reset_index().rename(columns={"Index": "Index4", 0: "deceased_id"})
df2_pivot.columns = ['variable', 'no_vent', 'vent']
df2_pivot['total_patient']=df2_pivot['no_vent']+df2_pivot['vent']
...
var_idcnt_ft2.to_csv(output_path+'/variable_summary_cycle1_vent.csv')
