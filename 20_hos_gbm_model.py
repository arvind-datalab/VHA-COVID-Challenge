...
features_df= pd.read_csv(output_path+'/variable_imp_hos.csv')
...
top_feature_df2 = top_feature_df1.append({'feature_name' : 'LOS_HOS'} , ignore_index=True)
...
model=GradientBoostingRegressor(n_estimators=100,learning_rate=0.1,criterion='mse',max_depth=7,loss='ls')
...
print("Mean squared error: %.2f"% sqrt(mean_squared_error(testy, y_pred)))
# Explained variance score: 1 is perfect prediction
...
feat_imp.to_csv(output_path+'/final_variable_list_hos.csv')
...
hos_output=pd.merge(covid_output_rename,result , how='left', on ='Id')
hos_output['prediction_new']=np.where(hos_output['covid_flag']==1,hos_output['prediction'],0)
final_result=hos_output.drop(['covid_prob','covid_flag','prediction'],axis=1)
final_result.to_csv(output_path+'/hos_output.csv', index = False,header=False)
